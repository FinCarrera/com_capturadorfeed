<?php
/* Edited by Matt Faulds to allow use of script within classes (external function calls within a class namespace) */
...
//function_exists($C['hook_tag']) ? jexit('yes'.$C['hook_tag']) : jexit('no'.$C['hook_tag']);
//jexit($C['hook_tag'][0].$C['hook_tag'][1]);
// config rest
...
$C['hook_tag'] = (!empty($C['hook_tag']) && method_exists($C['hook_tag'][0],$C['hook_tag'][1])) ? $C['hook_tag'] : 0;
...
if(empty($C['hook_tag'])){
 ...
 return call_user_func($C['hook_tag'],$e, $a);}
// eof
}
