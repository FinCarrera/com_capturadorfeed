<?php
/* Adjusted for use in Feed Gator by Matt Faulds. See "//FG edit" Last edit FG2.3 RC8 */
/** 
...
class Readability
{
	...
	public $debug = false; //FG edit
	public $debugMsg = ''; //FG edit
	protected $body = null; // 
	...
	*/
	...
	/**
	* PreClean restricts content analysed based on parameters - currently just the whitelist
	*/
	protected function preClean() {
		global $fgParams;
		...
		if(strpos($parts,'+') !== false) {
			...
			foreach($clean_config as $tag => $attribs) {
				$tl = $body->getElementsByTagName($tag);
				for($i=0;$i<$tl->length;$i++) {
					foreach($attribs as $k => $v) {
						if($tl->item($i)->getAttribute($k) AND in_array($tl->item($i)->getAttribute($k),$v)) {
							$tl->item($i)->getAttribute($k) ? $this->dbg('Cleaning Specifically: kept ' . $tag . ' (' . $k . ':' . $tl->item($i)->getAttribute($k) . ')') :  $this->dbg('Cleaning Specifically: kept ' . $tag . ' number '. $i) ;
							$nBody->appendChild($tl->item($i));
						}
					}
				}
			}
			$root->removeChild($body);
			$root->appendChild($nBody);
		}
	}
	...
	**/
	public function init()
	{
		...
		$relaxParsing	= $fgParams->get('relax_parsing');
		$articleContent = $relaxParsing ? $this->dom->createElement('div') : $this->grabArticle(); //FG edit
		if (!$articleContent->innerHTML) { //FG edit
			for($i=0;$i<3;$i++) {
		...
		for ($i=$curTagsLength-1; $i >= 0; $i--) {
			...
			if ($weight + $contentScore < 0) {
				...
				**/
				...
				if ( $img > $p ) {
//FG edit				//$toRemove = true; too punitive!!
